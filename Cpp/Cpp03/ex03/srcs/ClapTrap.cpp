...
void ClapTrap::takeDamage(unsigned int amount)
{
	...
	{
		_hitPoint -= amount;
		std::cout << _name << "'s energy point decreased to " << _hitPoint << std::endl;
		if (_hitPoint <= 0)
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (_hitPoint > 0 && _energyPoint > 0)
	{
		_energyPoint--;
		_hitPoint += amount;
		std::cout << _name << "'s hp restored to " << _hitPoint << std::endl;
	}
}
...
void	ClapTrap::powerUp(unsigned int amount)
{
	if (_hitPoint > 0 && _energyPoint > 0)
	{
		...
		std::cout << _name << "'s attack damage increased to " << _attackDamage << std::endl;
	}
}
