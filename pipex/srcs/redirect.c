...
/*   Updated: 2023/09/27 13:13:43 by hyuim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redirect_first_child(t_bundle *bundle, int fd[2])
{
	...
	if (infile_fd == -1)
		ft_error("Open Error ", 16);
	if (dup2(infile_fd, STDIN_FILENO) == -1)
		ft_error("Dup2 Error ", 17);
	if (dup2(fd[1], STDOUT_FILENO) == -1)
		ft_error("Dup2 Error ", 18);
	close(fd[1]);
	close(infile_fd);
}
...
void	redirect_mid_childs(int fd[2], int before_fd_read)
{
	...
	if (dup2(before_fd_read, STDIN_FILENO) == -1)
		ft_error("Dup2 Error ", 19);
	if (dup2(fd[1], STDOUT_FILENO) == -1)
		ft_error("Dup2 Error ", 20);
...
void	redirect_last_child(t_bundle *bundle, int fd[2], int before_fd_read)
{
	...
	if (outfile_fd == -1)
		ft_error("Open Error ", 21);
	if (dup2(before_fd_read, STDIN_FILENO) == -1)
		ft_error("Dup2 Error ", 22);
	if (dup2(outfile_fd, STDOUT_FILENO) == -1)
		ft_error("Dup2 Error ", 23);
	close(before_fd_read);
	close(outfile_fd);
}
