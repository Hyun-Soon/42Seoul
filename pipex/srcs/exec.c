...
/*   Updated: 2023/09/27 13:06:27 by hyuim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_cmd(t_bundle *bundle, char *envp[], int idx)
{
	...
	ft_error("Cmd not found Error ", 13);
}
...
void	exec_one_cmd(t_bundle *bundle, char *envp[])
{
	...
	if (infile_fd == -1)
		ft_error("Open Error ", 9);
	outfile_fd = open(bundle->out_filename, bundle->outfile_flag, 0644);
	if (outfile_fd == -1)
		ft_error("Open Error ", 10);
	if (dup2(infile_fd, STDIN_FILENO) == -1)
		ft_error("Dup2 Error ", 11);
	if (dup2(outfile_fd, STDOUT_FILENO) == -1)
		ft_error("Dup2 Error ", 12);
	close(infile_fd);
	close(outfile_fd);
...
void	exec_multiple_cmds(t_bundle *bundle, char *envp[], int idx)
{
	...
	while (++idx < bundle->cmd_cnts)
	{
		before_fd_read = fd[0];
		if (pipe(fd) == -1)
			ft_error("Pipe Error ", 14);
		pid = fork();
		if (pid < 0)
			ft_error("Fork Error ", 15);
		if (pid == 0)
		{
			if (idx == 0)
