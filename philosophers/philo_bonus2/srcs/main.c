/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   main.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hyuim <hyuim@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/24 14:05:40 by hyuim             #+#    #+#             */
/*   Updated: 2023/11/28 23:35:16 by hyuim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers_bonus.h"

void	null_init(t_bundle *bundle)
{
	bundle->sem_name = NULL;
	bundle->print_sem_name = NULL;
	//bundle->s_semaphore = NULL;
	//bundle->s_print_semaphore = NULL;
}

void	init_bundle(t_bundle *bundle, int argc, char **argv)
{
	null_init(bundle);
	bundle->eat_cnt = 0;
	bundle->num_of_philos = ft_atoi(argv[1]);
	bundle->time_to_die = ft_atoi(argv[2]);
	bundle->time_to_eat = ft_atoi(argv[3]);
	bundle->time_to_sleep = ft_atoi(argv[4]);
	bundle->number_of_times_for_each = -2;
	bundle->odd_flag = bundle->num_of_philos % 2;
	bundle->dead_flag = 0; //?
	if (argc == 6)
		bundle->number_of_times_for_each = ft_atoi(argv[5]);
	if (bundle->num_of_philos == -1
		|| bundle->number_of_times_for_each == -1
		|| bundle->time_to_die == -1
		|| bundle->time_to_eat == -1
		|| bundle->time_to_sleep == -1)
	{
		//printf("num_of_philos : %d\ntime_to_die : %d\ntime_to_eat : %d\ntime_to_sleep : %dnumber_of_for_each : %d\n", \
		//bundle->num_of_philos, bundle->time_to_die, bundle->time_to_eat, bundle->time_to_sleep, bundle->number_of_times_for_each);
		write(2, "arg err.\n", 9);
		exit(21);
	}
}

// general = 0
// starve = 1

//TODO ::	free list => 1. bundle.sem_name
//			usleep for every case after sleeping
//			***catch exit status and kill if the dead reason is hunger
//			even if a philo is full, until other all philos are full, the msg should be printed

void	check()
{
	system("leaks philo_bonus");
}

//cnt 인자가 있을 때, die 출력이 안됌.
//30 210 200 200을 줬을 때, 210에서 안죽고 400에서 죽음.
//1명일 때 처리.

int main(int argc, char **argv)
{
	t_bundle	bundle;

	if (argc < 5)
	{
		printf("Not enough args.\n");
		return (1);
	}
	init_bundle(&bundle, argc, argv);
	make_semaphores(&bundle);
	
	make_philosophers(&bundle);
	wait_philosophers(&bundle);
	delete_semaphores(&bundle);
	free_remains(&bundle);
	return (0);
}
